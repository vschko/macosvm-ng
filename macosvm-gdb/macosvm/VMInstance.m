...
@interface GDBStubDelegate : NSObject <_VZGDBDebugStubDelegate>
@end

@implementation GDBStubDelegate
- (void)_debugStub:(id)stub didStartListeningOnPort:(unsigned short)port {
    fprintf(stderr, "\n======================================================\n"
                    "  GDB stub listening on port %u\n"
                    "  Connect: lldb -o \"gdb-remote localhost:%u\"\n"
                    "======================================================\n\n",
            port, port);
}
@end
...
- (instancetype) configure {
    ...
    /* GDB debug stub (private API) — skip during restore/install */
#ifdef MACOS_GUEST
    BOOL isRestore = (self.restoreImage != nil);
#else
    BOOL isRestore = NO;
#endif
    if (gdb_port && !isRestore) {
        _VZGDBDebugStubConfiguration *gdbConfig =
            [[_VZGDBDebugStubConfiguration alloc] initWithPort:gdb_port];
        gdbConfig.listensOnAllNetworkInterfaces = gdb_listen_all;
        [self _setDebugStub:gdbConfig];
        NSLog(@" + GDB stub on port %u (%@)", gdb_port,
              gdb_listen_all ? @"all interfaces" : @"localhost");
    } else if (gdb_port && isRestore) {
        NSLog(@" + GDB stub skipped during restore (will activate on next boot)");
    }
...
- (void) start_ {
    void (^completionHandler)(NSError *errorOrNil) = ^(NSError *err) {
        ...
        /* Attach GDB delegate after successful start */
        if (_spec->gdb_port) {
            _VZDebugStub *stub = [_virtualMachine _debugStub];
            if (stub && [stub isKindOfClass:[_VZGDBDebugStub class]]) {
                _gdbDelegate = [[GDBStubDelegate alloc] init];
                ((_VZGDBDebugStub *)stub).delegate = _gdbDelegate;
                NSLog(@"GDB stub active on port %u", ((_VZGDBDebugStub *)stub).port);
            }
        }
